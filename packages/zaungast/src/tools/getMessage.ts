...
import { renderReactions, resolveConversationArg, whoLabel } from './readMessages.js';

// get_message = render(messages.get(convId, id)) at FULL width. Sibling of read_messages: that tool
...
export function getMessage(view: View, args: GetMessageArgs = {} as GetMessageArgs): string {
  ...
  const pivot =
    row.rootId && String(row.rootId) !== id
      ? `in thread m:${row.rootId} · read_messages(thread: m:${row.rootId})`
      : '';
