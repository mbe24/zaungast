...
use super::convert::{
    as_str, compact_attendees, compact_participants, compact_reactions, conv_kind, get,
    has_attachment, is_system_message, is_true, is_truthy, latin1_hex, mentioned_mris,
...
impl Handles {
    ...
    pub(crate) fn seed_from(&mut self, conn: &Connection) {
        for (table, id_col) in [("conversations", "id"), ("people", "mri")] {
            let mut stmt = conn
                .prepare(&format!(
                    "select {id_col}, handle from {table} where handle is not null"
                ))
                .unwrap();
            let rows = stmt
...
pub(crate) fn apply_conversation_meta(conn: &Connection, convs: &[Ssv], handles: &mut Handles) {
    ...
    for c in convs {
        ...
        let topic = as_str(get(c, "topic"))
            .filter(|s| !s.is_empty())
            .map(String::from); // c.topic || null
        let team_id = as_str(get(c, "teamId"))
            .filter(|s| !s.is_empty())
            .map(String::from);
        let thread_type = as_str(get(c, "threadType"))
            .filter(|s| !s.is_empty())
            .map(String::from);
        let meta_last = {
...
pub(crate) fn apply_messages(
    ...
    for m in msgs {
        ...
        let sender_mri = as_str(get(m, "senderId"))
            .filter(|s| !s.is_empty())
            .unwrap_or("?")
            .to_string();
        let is_mine =
            (self_mri == Some(sender_mri.as_str())) || is_true(get(m, "isSentByCurrentUser"));
        let mentions_me = self_mri.is_some()
            && !is_mine
            && mentioned_mris(m)
                .iter()
                .any(|x| Some(x.as_str()) == self_mri);
        let id_str = to_js_string(get(m, "id")).unwrap_or_default();
        let parent = to_js_string(get(m, "parentMessageId")).unwrap_or_default();
        let root_id = if !parent.is_empty() && parent != id_str {
            parent.clone()
        } else {
            id_str.clone()
        };
...
pub(crate) fn replace_events(conn: &Connection, rows: &[Ssv]) {
    ...
    for r in rows {
        ...
        let id = to_js_string(get(r, "id"))
            .unwrap_or_else(|| as_str(get(r, "__key")).unwrap_or("").to_string());
        let cid_raw = as_str(get(r, "cid"));
        let cid = cid_raw
            .filter(|c| c.contains("19:meeting_"))
            .map(String::from);
        let is_meeting = cid.is_some() || is_true(get(r, "isOnlineMeeting"));
        let is_confidential =
            is_truthy(get(r, "sensitivityLabelId")) || is_true(get(r, "doNotForward"));
        stmt.execute(params![
            id,
...
pub(crate) fn replace_calls(conn: &Connection, rows: &[Ssv]) {
    ...
    for r in rows {
        ...
        let counterpart = if direction.as_deref() == Some("Outgoing") {
            get(r, "target")
        } else {
            get(r, "originator")
        };
        ...
        let has_recording =
            matches!(get(r, "recordings"), Some(Ssv::Array { items, .. }) if !items.is_empty());
        ...
        stmt.execute(params![
            ...
            i64::from(get(r, "isCurrentUserPartOfCall") != Some(&Ssv::Bool(false))),
...
pub(crate) fn recompute_derived(conn: &Connection, self_mri: Option<&str>, handles: &mut Handles) {
    ...
    {
        let mut ins = conn
            .prepare("insert into conversations(id,handle,kind) values(?,?,?)")
            .unwrap();
        for (conv_id, kind) in &missing {
            ins.execute(params![conv_id, handles.handle_for('c', conv_id), kind])
                .unwrap();
        }
    }
    ...
    };
    {
        let mut name_stmt = conn
            ...
            .unwrap();
        let mut ins = conn
            .prepare("insert into people(mri,handle,name,msg_count,last_ts) values(?,?,?,?,?)")
            .unwrap();
        for (mri, c, last) in &ppl {
            let name: String = name_stmt
                .query_row(params![mri], |r| r.get::<_, Option<String>>(0))
                .ok()
                .flatten()
                .unwrap_or_default();
            ins.execute(params![mri, handles.handle_for('p', mri), name, c, last])
                .unwrap();
        }
    }
