...
interface StoreInfo {
  key: string; dbId: number; dbName: string; dbNorm: string; store: string
  count: number; sampled: number; fields: Set<string>; nested?: Set<string>; nestedUnder?: string
}

const normalizeDbName = (n: string) => n
  .replace(/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/gi, '<guid>')
  .replace(/:[a-z]{2}-[a-z]{2}$/i, ':<locale>').replace(/:\d+:/g, ':<n>:').replace(/_\d+_/g, '_<n>_')
...
const MSG_FIELDS: Record<string, string[]> = {
  ...
  time: ['originalArrivalTime', 'composeTime', 'clientArrivalTime', 'createdTime', 'timestamp', 'arrivalTime'],
...
function proposeEntity(fields: Set<string>, spec: Record<string, string[]>): { map: Record<string, string>; hits: number } {
  const map: Record<string, string> = {}; let hits = 0
  for (const [logical, cands] of Object.entries(spec)) { const f = pick(fields, cands); if (f) { map[logical] = f; hits++ } }
  return { map, hits }
}
...
export function describeSchema(dir: string, args: any = {}): string {
  ...
  const dbNames = new Map<number, string>(); const storeNames = new Map<string, string>()
  for (const { key, value } of live) {
    if (key.length < 1) continue
    let p: any; try { p = decodePrefix(key) } catch { continue }
    if (p.databaseId === 0 && p.objectStoreId === 0 && p.indexId === 0 && key[p.headerLen] === 0xc9) {
      const [, p2] = readStringWithLength(key, p.headerLen + 1); const [name] = readStringWithLength(key, p2)
  ...
  for (const { key, value } of live) {
    ...
    if (!info) {
      ...
      info = { key: sk, dbId: p.databaseId, dbName, dbNorm: normalizeDbName(dbName), store: storeNames.get(sk) ?? '?', count: 0, sampled: 0, fields: new Set() }
      stores.set(sk, info)
    }
    info.count++
    if (info.sampled < CAP) {
            ...
            if (c && typeof c === 'object' && !Array.isArray(c)) {
              for (const entry of Object.values(c).slice(0, 3)) {
                if (entry && typeof entry === 'object') {
                  info.nested ??= new Set(); info.nestedUnder = container
  ...
  // score candidates
  const msgCand = all.map((s) => ({ s, p: proposeEntity(s.nested ?? s.fields, MSG_FIELDS) }))
    .filter((x) => x.p.hits >= 3 && x.p.map.content).sort((a, b) => b.p.hits - a.p.hits)
  const convScore = (x: any) => x.p.hits + (/^conversations?$/i.test(x.s.store) ? 100 : /conversation/i.test(x.s.store) ? 10 : 0)
  const convCand = all.map((s) => ({ s, p: proposeEntity(s.fields, CONV_FIELDS) }))
    .filter((x) => x.p.hits >= 2 && (x.p.map.topic || /conversation/i.test(x.s.store))).sort((a, b) => convScore(b) - convScore(a))
  ...
  const INTEREST = new Set([...Object.values(MSG_FIELDS).flat(), ...Object.values(CONV_FIELDS).flat()])
  const showFields = (set: Set<string>): string => {
    const arr = [...set]
    return [...arr.filter((f) => INTEREST.has(f)), ...arr.filter((f) => !INTEREST.has(f))].slice(0, 24).join(', ') || '(none decoded)'
  }
  ...
  const buildEntity = (cand: any, iterate?: string) => {
    if (!cand) return null
    const e: any = { db: `*${cand.s.dbNorm.replace(/^Teams:/, '').split(':')[0]}*`, store: cand.s.store, fields: cand.p.map }
    if (iterate) { e.iterate = `${iterate}.*`; e.keep = { field: 'type', equals: 'Message' } }
    return e
  }
  const proposal: any = { schemaVersion: 'teams-PROPOSED', knownFingerprints: [fp.hash], match: { requireStores: [] }, entities: {} }
  if (msgCand[0]) { proposal.entities.message = buildEntity(msgCand[0], msgCand[0].s.nestedUnder); proposal.match.requireStores.push(msgCand[0].s.store) }
  if (convCand[0]) { proposal.entities.conversation = buildEntity(convCand[0]); proposal.match.requireStores.push(convCand[0].s.store) }
  ...
  lines.push('')
  lines.push('This is a proposal only — nothing was applied. Confirm the field paths against real records, then add the file and restart.')
  return lines.join('\n')
}
