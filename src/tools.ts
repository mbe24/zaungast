...
import type {
  ListConversationsArgs, ReadMessagesArgs, SearchArgs, TopTopicsArgs, FindPersonArgs,
} from './schemas.js'
...
export function parseTime(s: string | number | undefined, now = Date.now()): number | undefined {
  ...
  if (rel) { const n = +rel[1]; const u = rel[2]; return now - n * (u === 'd' ? 864e5 : u === 'h' ? 36e5 : 6e4) }
  const t = Date.parse(s); return Number.isNaN(t) ? undefined : t
}
...
function likeEscape(s: string): string { return s.replace(/[\\%_]/g, (m) => '\\' + m) }
...
function convIdsFor(db: DB, arg: string): string[] {
  ...
  return (db.prepare(`select id from conversations where topic like ? escape '\\' or participant_names like ? escape '\\'`)
...
function resolveExcludes(db: DB, list: string[] | undefined): { convIds: string[]; mris: string[]; words: string[]; miss?: string } {
  const convIds: string[] = [], mris: string[] = [], words: string[] = []
  for (const raw of list ?? []) {
    const e = String(raw).trim(); if (!e) continue
    if (e.startsWith('c:')) { const r = db.prepare('select id from conversations where handle=?').get(e) as any; if (!r) return { convIds, mris, words, miss: `exclude: no conversation ${e}` }; convIds.push(r.id) }
    else if (e.startsWith('p:')) { const r = db.prepare('select mri from people where handle=?').get(e) as any; if (!r) return { convIds, mris, words, miss: `exclude: no person ${e}` }; mris.push(r.mri) }
...
function coverageNote(db: DB, scopeSql: string, scopeParams: any[]): string {
  const r = db.prepare(`select min(m.ts) lo, max(m.ts) hi from messages m where ${scopeSql} and m.ts>0`).get(...scopeParams) as any
...
function convAmbiguityNote(db: DB, arg: string, ids: string[]): string {
  ...
  const rows = db.prepare(`select handle,topic,participant_names from conversations where id in (${ids.map(() => '?').join(',')}) order by last_ts desc limit 4`).all(...ids) as any[]
...
export function listConversations(store: ChatStore, meta: StoreMeta, deferred: boolean, args: ListConversationsArgs = {}): string {
  const db = store.db
  const bt = badTime(args, ['since']); if (bt) return bt
  ...
  if (args.query) { where.push('(topic like ? or participant_names like ?)'); params.push(`%${args.query}%`, `%${args.query}%`) }
  if (args.participant) { where.push('participant_names like ?'); params.push(`%${args.participant}%`) }
  const since = parseTime(args.since); if (since) { where.push('last_ts>=?'); params.push(since) }
  const w = where.length ? 'where ' + where.join(' and ') : ''
  const rows = db.prepare(`select handle,kind,topic,participant_names,last_ts,msg_count
     from conversations ${w} order by last_ts desc limit ?`).all(...params, n) as any[]
...
export function readMessages(store: ChatStore, meta: StoreMeta, deferred: boolean, args: ReadMessagesArgs = {} as ReadMessagesArgs): string {
  const db = store.db
  const bt = badTime(args, ['since', 'until']); if (bt) return bt
  ...
  if (ids.length > 1 && !String(args.conversation).startsWith('c:')) {
    const cands = db.prepare(`select handle,kind,topic,participant_names from conversations
      where id in (${ids.map(() => '?').join(',')}) order by last_ts desc limit 8`).all(...ids) as any[]
    return `ambiguous — ${ids.length} conversations match. Pick a handle:\n` +
      cands.map((c) => `  ${c.handle} [${c.kind}] ${c.topic || c.participant_names}`).join('\n')
  }
  const id = ids[0]
  const limit = Math.min(Number(args.limit) || 40, 200)
  const conds = ['conv_id=?', 'is_system=0']; const p: any[] = [id]
  const since = parseTime(args.since); if (since) { conds.push('ts>=?'); p.push(since) }
  const until = parseTime(args.until); if (until) { conds.push('ts<?'); p.push(until) }
  ...
  if (older) { conds.push('(ts<? or (ts=? and id<?))'); p.push(Number(older[1]), Number(older[1]), older[2]) }
  ...
  if (args.around) {
    ...
    const before = db.prepare(`select * from messages where conv_id=? and is_system=0 and ts<=? order by ts desc, id desc limit ?`).all(id, a.ts, half) as any[]
    const after = db.prepare(`select * from messages where conv_id=? and is_system=0 and ts>? order by ts asc, id asc limit ?`).all(id, a.ts, half) as any[]
    rows = [...before.reverse(), ...after]
  } else {
    // last `limit` in the window, rendered oldest→newest (story order)
    rows = (db.prepare(`select * from messages where ${conds.join(' and ')} order by ts desc, id desc limit ?`).all(...p, limit) as any[]).reverse()
  }
  const conv = db.prepare('select handle,kind,topic,participant_names from conversations where id=?').get(id) as any
  const total = (db.prepare('select count(*) n from messages where conv_id=? and is_system=0').get(id) as any).n
  const span = (db.prepare('select min(ts) lo, max(ts) hi from messages where conv_id=? and is_system=0 and ts>0').get(id) as any)
  const earliest = span?.lo ?? 0, newest = span?.hi ?? 0
  ...
  let lastMri: string | null = null, lastTs = 0
  const lines = rows.map((r) => {
    ...
    lastMri = r.sender_mri; lastTs = r.ts
...
export function search(store: ChatStore, meta: StoreMeta, deferred: boolean, args: SearchArgs = {}): string {
  const db = store.db
  const bt = badTime(args, ['since', 'until']); if (bt) return bt
  ...
  const scopeConds: string[] = ['m.is_system=0']; const scopeParams: any[] = []
  const notes: string[] = []
  if (args.from) {
    const f = senderFilter(db, String(args.from)); if (f.miss) return f.miss; scopeConds.push(f.sql); scopeParams.push(...f.params)
    if (!String(args.from).startsWith('p:')) {
      const ppl = db.prepare("select handle,name from people where name like ? escape '\\' order by msg_count desc").all(`%${likeEscape(String(args.from))}%`) as any[]
      if (ppl.length > 1) notes.push(`note: from:"${args.from}" matched ${ppl.length} people (${ppl.slice(0, 4).map((p) => `${p.handle} ${p.name}`).join(', ')}) — pass a p:handle to narrow`)
    }
  }
  if (args.in) {
    const ids = convIdsFor(db, String(args.in)); if (!ids.length) return `no conversation matches "${args.in}"`
    scopeConds.push(`m.conv_id in (${ids.map(() => '?').join(',')})`); scopeParams.push(...ids)
    const amb = convAmbiguityNote(db, String(args.in), ids); if (amb) notes.push(amb)
  }
  if (args.kind) { scopeConds.push('m.kind=?'); scopeParams.push(args.kind) }
  ...
  if (args.exclude?.length) {
    const ex = resolveExcludes(db, args.exclude); if (ex.miss) return ex.miss
    if (ex.convIds.length) { scopeConds.push(`m.conv_id not in (${ex.convIds.map(() => '?').join(',')})`); scopeParams.push(...ex.convIds) }
    if (ex.mris.length) { scopeConds.push(`m.sender_mri not in (${ex.mris.map(() => '?').join(',')})`); scopeParams.push(...ex.mris) }
  }

  const conds = [...scopeConds]; const params = [...scopeParams]
  const since = parseTime(args.since); if (since) { conds.push('m.ts>=?'); params.push(since) }
  const until = parseTime(args.until); if (until) { conds.push('m.ts<?'); params.push(until) }
  ...
  if (match && meta.ftsEnabled) {
    order = 'relevance'
    rows = db.prepare(`select m.*, snippet(messages_fts,0,'[',']','…',10) snip
      from messages_fts f join messages m on m.conv_id=f.conv_id and m.id=f.id
      where messages_fts match ? and ${conds.join(' and ')}
      order by bm25(messages_fts) limit ?`).all(match, ...params, limit) as any[]
  } else {
    if (args.query && String(args.query).trim()) { conds.push('m.content like ? escape \'\\\''); params.push(`%${likeEscape(String(args.query))}%`) }
    order = 'time'
    rows = db.prepare(`select m.*, substr(m.content,1,120) snip from messages m
      where ${conds.join(' and ')} order by m.ts desc, m.id desc limit ?`).all(...params, limit) as any[]
  }
  const convH = new Map<string, string>()
  const hn = (cid: string) => convH.get(cid) ?? convH.set(cid, (db.prepare('select handle from conversations where id=?').get(cid) as any)?.handle ?? '?').get(cid)!
  const distinct = [...new Set(rows.map((r) => r.conv_id))]
  const legend = distinct.length <= 5
    ? distinct.map((cid) => `${hn(cid)}="${(db.prepare('select topic,participant_names from conversations where id=?').get(cid) as any)?.topic || (db.prepare('select participant_names from conversations where id=?').get(cid) as any)?.participant_names || '?'}"`).join(' · ')
    : ''
  const lines = rows.map((r) => `${hn(r.conv_id)} ${fmtTs(r.ts)} m:${r.id} ${r.is_mine ? 'ME' : r.sender_name}> ${clip(r.snip, 140)}`)
  ...
  if (rows.length === 0) coverage = coverageNote(db, scopeSql, scopeParams)
    ...
    const hi = (db.prepare(`select max(m.ts) hi from messages m where ${scopeSql}`).get(...scopeParams) as any)?.hi ?? 0
    if (since > hi) coverage = coverageNote(db, scopeSql, scopeParams)
  }

  const head = [envelope(meta, deferred, `order:${order} · ${rows.length} hits`), ...notes, legend, coverage].filter(Boolean).join('\n')
  return `${head}\n${lines.join('\n') || '(no matches)'}`
}

export function topTopics(store: ChatStore, meta: StoreMeta, deferred: boolean, args: TopTopicsArgs = {}): string {
  const db = store.db
  const bt = badTime(args, ['since', 'until']); if (bt) return bt
  ...
  for (const r of db.prepare('select name from people').all() as any[])
    for (const w of String(r.name || '').toLowerCase().match(/[\p{L}\p{M}]{3,}/gu) || []) nameTokens.add(w)
  ...
  if (store.phraseCacheSig !== sig) { store.phraseCache.clear(); store.phraseCacheSig = sig }
  const cache = store.phraseCache
  const ex = resolveExcludes(db, args.exclude); if (ex.miss) return ex.miss
  const excludeWords = new Set(ex.words)
  const phrases = (content: string): string[] => {
    let p = cache.get(content); if (!p) { p = extract(content); cache.set(content, p) }
    return excludeWords.size ? p.filter((ph) => !ph.split(' ').some((w) => excludeWords.has(w))) : p
  }
  ...
  const conds = ['is_system=0', "content<>''"]; const params: any[] = []
  let personScope = false
  if (args.scope && String(args.scope).startsWith('conversation:')) {
    const term = String(args.scope).slice(13)
    const ids = convIdsFor(db, term); if (!ids.length) return `no conversation matches "${term}"`
    conds.push(`conv_id in (${ids.map(() => '?').join(',')})`); params.push(...ids)
    const amb = convAmbiguityNote(db, term, ids); if (amb) notes.push(amb.replace('in:', 'scope conversation:'))
  } else if (args.scope && String(args.scope).startsWith('person:')) {
    const f = senderFilter(db, String(args.scope).slice(7)); if (f.miss) return f.miss
    conds.push(f.sql.replace('m.', '')); params.push(...f.params); personScope = true
  }
  if (ex.convIds.length) { conds.push(`conv_id not in (${ex.convIds.map(() => '?').join(',')})`); params.push(...ex.convIds) }
  if (ex.mris.length) { conds.push(`sender_mri not in (${ex.mris.map(() => '?').join(',')})`); params.push(...ex.mris) }
  const minSenders = personScope ? 1 : 2

  let all = db.prepare(`select ts, sender_mri, content from messages where ${conds.join(' and ')}`).all(...params) as any[]
  ...
  let botExcluded = 0
  if (!args.include_bots) { const before = all.length; all = all.filter((m) => !isBotMri(m.sender_mri)); botExcluded = before - all.length }
  ...
  let maxTs = 0; for (const m of all) if (m.ts > maxTs) maxTs = m.ts
  const windowMs = { '1d': 864e5, '7d': 7 * 864e5, '30d': 30 * 864e5 }[String(args.window || '7d')] ?? 7 * 864e5
  const sinceTs = explicit ? (parseTime(args.since) ?? 0) : maxTs - windowMs
  const untilTs = explicit ? (parseTime(args.until) ?? Number.MAX_SAFE_INTEGER) : Number.MAX_SAFE_INTEGER

  const baseDf = new Map<string, number>()
  let baseTotal = 0
  for (const m of all) if (m.ts < sinceTs) { baseTotal++; for (const ph of new Set(phrases(m.content))) baseDf.set(ph, (baseDf.get(ph) || 0) + 1) }

  const count = new Map<string, number>(), df = new Map<string, number>()
  const senders = new Map<string, Set<string>>(), example = new Map<string, any>()
  const win = all.filter((m) => m.ts >= sinceTs && m.ts < untilTs)
  for (const m of win) {
    const seen = new Set<string>()
    for (const ph of phrases(m.content)) {
      count.set(ph, (count.get(ph) || 0) + 1)
      if (!seen.has(ph)) { df.set(ph, (df.get(ph) || 0) + 1); seen.add(ph) }
      ;(senders.get(ph) || senders.set(ph, new Set()).get(ph)!).add(m.sender_mri)
      if (!example.has(ph)) example.set(ph, m)
    }
  }
  const rows = [...count.entries()].map(([ph, c]) => {
  ...
  if (botExcluded) notes.push(`excluded ${botExcluded} bot/app msgs · include_bots:true to include`)
...
export function findPerson(store: ChatStore, meta: StoreMeta, deferred: boolean, args: FindPersonArgs = {}): string {
  const db = store.db
  const n = Math.min(Number(args.n) || 8, 25)
  const q = args.query ? String(args.query).trim() : ''
  let rows: any[]; let header: string
  if (q.startsWith('p:')) {
    rows = db.prepare('select handle,mri,name,msg_count,last_ts from people where handle=?').all(q) as any[]
    if (!rows.length) return `${envelope(meta, deferred)}\nno person with handle ${q}`
    header = 'profile'
  } else if (q) {
    rows = db.prepare("select handle,mri,name,msg_count,last_ts from people where name like ? escape '\\' order by msg_count desc limit ?")
    ...
    rows = db.prepare('select handle,mri,name,msg_count,last_ts from people order by msg_count desc limit ?').all(n) as any[]
