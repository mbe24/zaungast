...
export interface Ingested { store: ChatStore; meta: StoreMeta; state: IngestState | null; lossy: boolean }

function loadMappings() {
  return fs.readdirSync(VERSIONS_DIR).filter((f) => f.endsWith('.json')).map((f) => loadMapping(path.join(VERSIONS_DIR, f)))
}
...
function applyMessages(store: ChatStore, msgRows: any[], selfMri: string | null): Set<string> {
  ...
  for (const m of msgRows) {
    ...
    store.insertMessage({
      convId, id: String(m.id), chainKey: m.__key, version: Number(m.version) || 0, ts,
      senderMri, senderName: m.senderName || '', kind: convKind(convId), isMine,
      isSystem: isSystemMessage(m) ? 1 : 0, hasAttach: hasAttachment(m, rawHtml) ? 1 : 0,
      mentionsMe, content: htmlToText(rawHtml),
    })
...
function applyConversationMeta(store: ChatStore, convRows: any[]) {
  for (const c of convRows) {
    ...
    store.upsertConversationMeta({
      id: c.id, kind: convKind(c.id), topic: c.topic || null,
      teamId: c.teamId || null, metaLastTs: Number(c.lastMessageTimeUtc) || 0,
    })
  }
}
...
function voteSelfMri(msgRows: any[]): string | null {
  ...
  for (const m of msgRows) if (m.isSentByCurrentUser && m.senderId) votes.set(m.senderId, (votes.get(m.senderId) || 0) + 1)
  return [...votes.entries()].sort((a, b) => b[1] - a[1])[0]?.[0] ?? null
}

function finalMeta(store: ChatStore, fp: any, mapping: any, mode: 'full' | 'incremental', lastFullAt: number, lossy: boolean, selfMri: string | null): StoreMeta {
  const c = store.counts()
  return {
    asOf: Date.now(), fingerprint: fp.hash, schemaVersion: mapping?.schemaVersion ?? null,
    schemaMatched: true, counts: { conversations: c.conversations, messages: c.messages, people: c.people },
    earliestTs: c.earliestTs, ftsEnabled: store.ftsEnabled, lastFullAt, refreshMode: mode, lossy, selfMri,
  }
}
...
export function ingest(dir: string, opts: { seqCap?: bigint } = {}): Ingested {
  ...
  if (!mapping) {
    ...
    return {
      store, state: null, lossy,
      meta: {
        asOf: Date.now(), fingerprint: fp.hash, schemaVersion: null, schemaMatched: false,
        counts: { conversations: 0, messages: 0, people: 0 }, earliestTs: 0, ftsEnabled: store.ftsEnabled,
        lastFullAt: Date.now(), refreshMode: 'full', lossy, selfMri: null,
      },
    }
  }
...
export interface LoadedEntries { live: any[]; maxSeq: bigint; lossy: boolean }

// Overload: accept a dir (reparse-load internally) or pre-loaded entries (copy-reuse path).
export function applyIncremental(store: ChatStore, state: IngestState, source: string | LoadedEntries):
  ...
  const newLive: any[] = []
  for (const e of live) {
    let p: any; try { p = decodePrefix(e.key) } catch { continue }
    if (p.indexId !== 1) { if (e.seq > state.maxSeq) newLive.push(e); continue }
  ...
  try {
    ...
    store.db.exec('delete from conversations where id not in (select id from _liveconv) and id not in (select distinct conv_id from messages)')
    store.db.exec('COMMIT')
    ...
    try { store.db.exec('ROLLBACK') } catch { /* already rolled back / already committed */ }
    return { needFullRebuild: true, newMaxSeq: state.maxSeq, skipped: false }
  }
