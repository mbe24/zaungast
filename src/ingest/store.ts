...
export class ChatStore {
  ...
  private detectFts(): boolean {
    try { const t = new DatabaseSync(':memory:'); t.exec('create virtual table _p using fts5(x)'); t.close(); return true }
    catch { return false }
  }
  ...
  handleFor(prefix: 'c' | 'p', fullId: string): string {
    ...
    for (let len = 6; len <= 40; len++) {
      ...
      if (!this.usedHandles.has(h)) { this.usedHandles.add(h); this.handleByFull.set(fullId, h); return h }
    }
    const fb = `${prefix}:${fullId.slice(0, 12)}`
    this.usedHandles.add(fb); this.handleByFull.set(fullId, fb); return fb
  }

  private stmt = new Map<string, ReturnType<DatabaseSync['prepare']>>()
  private q(sql: string) { let s = this.stmt.get(sql); if (!s) { s = this.db.prepare(sql); this.stmt.set(sql, s) } return s }

  upsertConversationMeta(c: { id: string; kind: string; topic: string | null; teamId: string | null; metaLastTs: number }) {
    this.q(`insert into conversations(id,handle,kind,topic,team_id,meta_last_ts) values(?,?,?,?,?,?)
  ...
  insertMessage(m: {
    convId: string; id: string; chainKey: string; version: number; ts: number; senderMri: string
    senderName: string; kind: string; isMine: number; isSystem: number; hasAttach: number; mentionsMe: number; content: string
  }) {
    ...
    this.q(`insert into messages
      ...
      .run(m.convId, m.id, m.chainKey, m.version, m.ts, m.senderMri, m.senderName, m.kind,
        m.isMine, m.isSystem, m.hasAttach, m.mentionsMe, m.content)
  }
  ...
  recomputeDerived(selfMri: string | null) {
    ...
    const missing = this.db.prepare(`select conv_id, min(kind) kind from messages
      where conv_id not in (select id from conversations) group by conv_id order by conv_id`).all() as any[]
    ...
    const ppl = this.db.prepare(`select sender_mri mri, count(*) c, max(ts) last from messages
      where is_system=0 group by sender_mri order by sender_mri`).all() as any[]
    const nameOf = this.db.prepare(`select sender_name from messages where sender_mri=? and is_system=0 order by ts desc, id desc limit 1`)
    ...
    this.db.exec('update conversations set last_ts=max(coalesce(meta_last_ts,0), coalesce(activity_ts,0))')
    ...
    this.q(`update conversations set participant_names=(
      select group_concat(name, ', ') from (
        ...
        group by m.sender_mri order by mts desc, m.sender_mri limit 5))`).run(self)
  }
  ...
  refreshFts(changedIds: Set<string> | null) {
    ...
    if (changedIds === null) {
      ...
      this.db.exec(`insert into messages_fts(content,conv_id,id) select content,conv_id,id from messages where is_system=0 and content<>''`)
      return
    }
    ...
    this.db.exec('BEGIN'); for (const id of changedIds) ins.run(id); this.db.exec('COMMIT')
