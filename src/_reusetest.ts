...
function liveSig(live: any[]): string {
  return live.map((e) => `${e.key.toString('latin1')}#${e.seq}#${e.type}`).sort().join('|')
}
...
console.log('=== A. cached loader output == reparse loader output (cold cache) ===')
{
  ...
  ok('cache populated with every .ldb', cache.size === fs.readdirSync(DIR).filter((f) => f.endsWith('.ldb')).length)
...
console.log('\n=== B. warm cache: second call reuses parses, identical output ===')
{
  ...
  ok('cache objects reused (same references)', [...cache.values()].every((v, i) => v === cachedObjs[i]))
}

console.log('\n=== C. spine: partial + applyIncremental(cached load) == full rebuild ===')
{
  const cap = (() => { const s = loadEntries(DIR).live.map((e: any) => e.seq).sort((a: bigint, b: bigint) => (a < b ? -1 : 1)); return s[Math.floor(s.length / 2)] })()
  const partial = ingest(DIR, { seqCap: cap })
  const cache = new Map()
  const loaded = loadEntriesReuse(DIR, cache)
  const r = applyIncremental(partial.store, partial.state!, { live: loaded.live, maxSeq: loaded.maxSeq, lossy: loaded.lossy })
  const full = ingest(DIR)
  const dump = (s: any) => JSON.stringify({
    m: s.db.prepare('select conv_id,id,version,ts,sender_mri,is_system,content from messages order by conv_id,id').all(),
...
console.log('\n=== D. compaction detected when a cached .ldb disappears ===')
{
  ...
  const victim = fs.readdirSync(d).filter((f) => f.endsWith('.ldb')).sort()[0]
...
console.log('\n=== E. new .ldb flush is picked up; still == reparse ===')
{
  const d = copyDir(DIR)
  const cache = new Map()
  const ldbs = fs.readdirSync(d).filter((f) => f.endsWith('.ldb')).sort()
  ...
  fs.rmSync(held, { force: true }); fs.rmSync(d, { recursive: true, force: true })
}
...
function craftDeletionLog(userKey: Buffer, seq: bigint): Buffer {
  const batch = Buffer.concat([(() => { const s = Buffer.alloc(8); s.writeBigUInt64LE(seq); return s })(), (() => { const c = Buffer.alloc(4); c.writeUInt32LE(1); return c })(), Buffer.from([0]), varint(userKey.length), userKey])
  const type = Buffer.from([1]); const crc = Buffer.alloc(4); crc.writeUInt32LE(maskCrc(crc32c(Buffer.concat([type, batch]), 0, 1 + batch.length)))
...
function msgStoreKey(dir: string): Buffer | null {
  ...
  const mapping = selectMapping(fs.readdirSync(VDIR).filter((f: string) => f.endsWith('.json')).map((f: string) => loadMapping(path.join(VDIR, f))), fingerprint(live)).mapping
...
function storeDump(store: any): string {
  return JSON.stringify({
    m: store.db.prepare('select conv_id,id,version,ts,sender_mri,is_system,content from messages order by conv_id,id').all(),
    c: store.db.prepare('select id,topic,msg_count,participant_names from conversations order by id').all(),
...
console.log('\n=== G. Session copy-reuse == reparse after the same mutation (mode equivalence) ===')
{
  ...
  sR.refreshNow(true); sC.refreshNow(true)               // both warm (full)
  sC.refreshNow(false)                                    // copy-reuse: first incremental warms cache
  // identical mutation: tombstone a chain in each
  const seqR = loadEntries(dR).maxSeq, seqC = loadEntries(dC).maxSeq
  ...
  const mR = sR.refreshNow(false), mC = sC.refreshNow(false)
  ok('copy-reuse refresh is incremental', mC.refreshMode === 'incremental')
  ok('copy-reuse store == reparse store after deletion', storeDump(sR.getStore()) === storeDump(sC.getStore()))
  sR.dispose(); sC.dispose(); fs.rmSync(dR, { recursive: true, force: true }); fs.rmSync(dC, { recursive: true, force: true })
}
...
console.log('\n=== H. Session copy-reuse multi-step (add then delete) == reparse ===')
{
  ...
  const sR = new (Session as any)({ overrideDir: dR, minDebounceMs: 0, incrementalMode: 'reparse' })
  const sC = new (Session as any)({ overrideDir: dC, minDebounceMs: 0, incrementalMode: 'copy-reuse' })
  sR.refreshNow(true); sC.refreshNow(true); sC.refreshNow(false)
  // step 1: tombstone chain A
  const kR1 = msgStoreKey(dR)!, kC1 = msgStoreKey(dC)!
  ...
  sR.refreshNow(false); sC.refreshNow(false)
...
console.log('\n=== I. H1 end-to-end: compaction (merge .ldb) → copy-reuse store == reparse, no resurrection ===')
{
  ...
  const sC = new (Session as any)({ overrideDir: dC, minDebounceMs: 0, incrementalMode: 'copy-reuse' })
  const sR = new (Session as any)({ overrideDir: dR, minDebounceMs: 0, incrementalMode: 'reparse' })
  sC.refreshNow(true); sC.refreshNow(false)  // warm copy-reuse cache
  sR.refreshNow(true)
  ...
  const mC = sC.refreshNow(false), mR = sR.refreshNow(false)
  const dump = (s: any) => JSON.stringify(s.db.prepare('select conv_id,id from messages order by conv_id,id').all())
  ...
  ok('copy-reuse store == reparse store after compaction', dump(sC.getStore()) === dump(sR.getStore()))
  sC.dispose(); sR.dispose(); fs.rmSync(dC, { recursive: true, force: true }); fs.rmSync(dR, { recursive: true, force: true })
}
...
console.log('\n=== J. needFullRebuild in copy-reuse latches a full rebuild (pendingFull) ===')
{
  ...
  ok('store still valid after fallback', (s.getStore().db.prepare('select count(*) n from messages').get() as any).n === before)
  s.dispose(); fs.rmSync(d, { recursive: true, force: true })
}
...
console.log('\n=== K. partial-.ldb wedge (H-A/H-B): a truncated snapshot .ldb recovers within one refresh ===')
{
  ...
  const victim = fs.readdirSync(snapDir).filter((f) => f.endsWith('.ldb')).sort()[0]
  ...
  const full = ingest(d)
  ok('recovered full message set within one refresh (size-check re-copied the truncated .ldb)',
     n === (full.store.db.prepare('select count(*) n from messages').get() as any).n)
  full.store.close(); s.dispose(); fs.rmSync(d, { recursive: true, force: true })
}
...
console.log('\n=== L. backstop fires in copy-reuse mode ===')
{
  ...
  s.refreshNow(true)
  const modes = [s.refreshNow(false).refreshMode, s.refreshNow(false).refreshMode, s.refreshNow(false).refreshMode]
