...
import { decodePrefix, readStringWithLength, readVarint, utf16be, decodeValue } from './chromium/indexeddb.js'
import type { DecodedPrefix, Entry, ExtractedRow, FingerprintResult, Mapping, SelectMappingResult } from './types.js'

// Resolve a mapping file against a fingerprint, then extract entities generically.

export function loadMapping(path: string): Mapping { return JSON.parse(fs.readFileSync(path, 'utf8')) }
...
const glob = (pat: string, s: string): boolean => new RegExp('^' + pat.replace(/[.+?^${}()|[\]\\]/g, '\\$&').replace(/\*/g, '.*') + '$').test(s)
...
function schemaTables(live: Entry[]): { dbNames: Map<number, string>; storeNames: Map<string, string> } {
  const dbNames = new Map<number, string>(), storeNames = new Map<string, string>()
  for (const { key, value } of live) {
    if (key.length < 1) continue
    let p: DecodedPrefix
    try { p = decodePrefix(key) } catch { continue }
...
export function entityTargets(live: Entry[], mapping: Mapping | null, entityName: string): Set<string> {
...
export function extractEntity(live: Entry[], mapping: Mapping | null, entityName: string, targets?: Set<string>): ExtractedRow[] {
  const def = (mapping as Mapping).entities[entityName]
  ...
  const rowsFromRecord = (obj: unknown, recKey: string): ExtractedRow[] => {
    ...
    for (const item of Object.values(container as Record<string, unknown>)) {
      if (def.keep && (item as Record<string, unknown> | null | undefined)?.[def.keep.field] !== def.keep.equals) continue
  ...
  for (const { key, value } of live) {
    ...
    let obj: unknown
    try { obj = decodeValue(value) } catch { continue }
    if (!obj) continue
