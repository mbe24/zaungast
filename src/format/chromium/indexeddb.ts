...
import type {
  DecodedPrefix, DeserializeOptions, Entry, ExternalBlobMarker, LdbCache,
  LoadEntriesOptions, LoadEntriesResult, LoadEntriesReuseResult, SsvValue, TableReadResult, WalBatch,
} from '../types.js'
...
function foldBatch(batch: WalBatch, consider: Consider): void {
  batch.ops.forEach((op, i) => consider(op.key, op.value, BigInt(batch.sequence) + BigInt(i), op.type))
}
...
export function loadEntries(dir: string, { includeLog = true, seqCap }: LoadEntriesOptions = {}): LoadEntriesResult {
  ...
  for (const f of files) {
    ...
    try { res = readTable(path.join(dir, f)) } catch (e) { console.error(`skip ${f}: ${(e as Error).message}`); lossy = true; continue }
  ...
  if (includeLog) {
    ...
    for (const lf of logFiles) {
      ...
      try { batches = parseWriteAheadLog(path.join(dir, lf)) } catch (e) { console.error(`skip log ${lf}: ${(e as Error).message}`); lossy = true; continue }
...
export function decodeValue(value: Buffer | null, opts: DeserializeOptions = {}): SsvValue | ExternalBlobMarker {
