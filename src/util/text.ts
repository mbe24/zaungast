...
const ENTITIES: Record<string, string> = {
  ...
  '&auml;': 'ä', '&ouml;': 'ö', '&uuml;': 'ü', '&szlig;': 'ß', '&Auml;': 'Ä', '&Ouml;': 'Ö', '&Uuml;': 'Ü',
  '&eacute;': 'é', '&egrave;': 'è', '&ecirc;': 'ê', '&agrave;': 'à', '&acirc;': 'â', '&ccedil;': 'ç',
  '&ntilde;': 'ñ', '&uacute;': 'ú', '&iacute;': 'í', '&oacute;': 'ó', '&aacute;': 'á', '&ldquo;': '"',
  '&rdquo;': '"', '&lsquo;': "'", '&rsquo;': "'", '&hellip;': '…', '&mdash;': '—', '&ndash;': '–', '&euro;': '€',
}
...
function codePoint(n: number): string { try { return String.fromCodePoint(n) } catch { return ' ' } }
...
export function mentionedMris(mentions: any): string[] {
  let raw = mentions
  if (raw && typeof raw === 'object' && !Array.isArray(raw) && raw.properties) raw = raw.properties.mentions
  let arr: any = raw
