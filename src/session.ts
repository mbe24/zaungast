...
function safeCopy(src: string, dest: string): void {
  ...
  try {
    ...
    try {
      ...
      let pos = 0, n: number
      while ((n = fs.readSync(inFd, buf, 0, buf.length, pos)) > 0) { fs.writeSync(outFd, buf, 0, n); pos += n }
...
function probeFingerprint(dir: string): string | null {
  try {
    const logs = fs.readdirSync(dir).filter((f) => f.endsWith('.log')).map((f) => {
      const st = fs.statSync(path.join(dir, f)); return `${f}:${st.size}:${st.mtimeMs}`
    }).sort().join('|')
    const cur = fs.existsSync(path.join(dir, 'CURRENT')) ? fs.statSync(path.join(dir, 'CURRENT')).mtimeMs : 0
    return `${logs}#${cur}`
  } catch { return null }
}
...
export class Session {
  ...
  private resolveLiveDir(): string {
    ...
    if (!found.length) throw new Error('No Teams IndexedDB found. Set TEAMS_LEVELDB_DIR to the leveldb dir.')
    return found[0].dir
  }
  ...
  private snapshotReuse(liveDir: string, dir: string): boolean {
    ...
    for (const f of live) {
      const src = path.join(liveDir, f), dst = path.join(dir, f)
      if (f.endsWith('.ldb')) {
        ...
        try {
          const ss = fs.statSync(src), ds = fs.existsSync(dst) ? fs.statSync(dst) : null
          if (ds && ds.size === ss.size) continue
        } catch { /* src vanished → copy will fail → lossy */ }
  ...
  private tryCopyReuse(): boolean {
    ...
    const now = Date.now()
    if (this.incrementalsSinceFull >= this.maxIncrementals || now - this.lastFullAt > this.fullIntervalMs) return false

    let liveDir: string
    try { liveDir = this.resolveLiveDir() } catch { return false }
    ...
    let compacted = false
    for (const f of this.ldbCache.keys()) if (!liveLdb.has(f)) { this.ldbCache.delete(f); compacted = true }
    ...
    try {
      ...
      if (lossy || loaded.lossy) { this.lastIngestAt = Date.now(); return true } // skip, keep current, retry
      if (loaded.compacted) return false // secondary safety net (pre-check should have caught it)

      const r = applyIncremental(this.cur.store, this.cur.state, { live: loaded.live, maxSeq: loaded.maxSeq, lossy: false })
      if (r.skipped) { this.lastIngestAt = Date.now(); return true }
      ...
      if (r.needFullRebuild) { this.pendingFull = true; return false }
      this.cur.state.maxSeq = r.newMaxSeq
      this.incrementalsSinceFull++
      const c = this.cur.store.counts()
      this.cur.meta = {
        ...this.cur.meta, asOf: Date.now(), refreshMode: 'incremental', lastFullAt: this.lastFullAt,
        counts: { conversations: c.conversations, messages: c.messages, people: c.people }, earliestTs: c.earliestTs, lossy: false,
      }
      this.lastProbe = probeFingerprint(liveDir)
      this.lastRebuildMs = Date.now() - t0
      this.lastIngestAt = Date.now()
      return true
    } catch { this.pendingFull = true; return false } // any throw → defer to a reparse FULL rebuild
  }
  ...
  private refresh(forceFull: boolean): void {
    ...
    const t0 = Date.now()
    let dir: string, prevSnap: string | null = null, liveDir: string | null = null, snapLossy = false
    if (this.staticDir) { dir = this.staticDir }
    else { liveDir = this.resolveLiveDir(); const s = this.snapshot(liveDir); dir = s.dir; snapLossy = s.lossy; prevSnap = this.snapshotDir }
    ...
    const doFull = (): void => {
      ...
      if (lossy && this.cur?.meta.schemaMatched) { next.store.close(); return } // keep good store
    ...
    const wantFull = forceFull || this.pendingFull || !this.cur || !this.cur.state ||
      this.incrementalsSinceFull >= this.maxIncrementals || now - this.lastFullAt > this.fullIntervalMs
    ...
    try {
      if (wantFull) {
        ...
        try {
          ...
          if (r.skipped) { handled = true }              // lossy read → keep current, retry
          else if (r.needFullRebuild) { doFull(); handled = true }
          else {
            this.cur!.state!.maxSeq = r.newMaxSeq
            this.incrementalsSinceFull++
            const c = this.cur!.store.counts()
            this.cur!.meta = {
              ...this.cur!.meta, asOf: Date.now(), refreshMode: 'incremental', lastFullAt: this.lastFullAt,
              counts: { conversations: c.conversations, messages: c.messages, people: c.people }, earliestTs: c.earliestTs,
              lossy: false,
            }
            caughtUp = true; handled = true
          }
        } catch { /* R2: any throw → full rebuild; never leave a half-mutated store */ }
        if (!handled) doFull()
      }
    } finally {
  ...
  private debounceMs(): number { return Math.max(this.minDebounceMs, 5 * this.lastRebuildMs) }

  // Kick off the first (full) ingest eagerly after the MCP handshake.
  warmUp(): void { if (!this.cur) { try { this.refresh(true) } catch { /* first get() will surface it */ } } }

  get(): Ingested & { staleProbeDeferred: boolean } {
    if (!this.cur) { this.refresh(true); return { ...this.cur!, staleProbeDeferred: false } }
    let deferred = false
    if (!this.staticDir) {
      try {
  ...
  currentDir(): string {
    ...
    if (!this.snapshotDir) { this.refresh(true) }
...
function copyFileWithRetry(src: string, dest: string): boolean {
  for (let attempt = 0; attempt < 3; attempt++) {
    try { safeCopy(src, dest); return true }
    catch (e: any) {
      try { fs.rmSync(dest, { force: true }) } catch { /* nothing to clean */ }
      if (['ENOENT', 'EBUSY', 'EPERM', 'EACCES'].includes(e.code)) { if (attempt < 2) continue; return false }
      throw e
    }
  }
  return false
