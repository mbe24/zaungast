...
import {
  listConversationsShape, readMessagesShape, searchShape, topTopicsShape, findPersonShape, describeSchemaShape,
} from './schemas.js'

const HISTORY_NOTE = 'Note: this reads the LOCAL Teams cache — history is the synced slice on this device, not the full server archive.'
...
export function buildServer(session: Session): McpServer {
  ...
  const run = (fn: (s: any, m: any, d: boolean, a: any) => string, args: any) => {
    try {
      ...
      if (!meta.schemaMatched) {
        return { content: [{ type: 'text' as const, text: `This Teams build's IndexedDB schema is not recognized (fingerprint ${meta.fingerprint}). Run the describe_schema tool to inspect the stores and get a proposed mapping.` }] }
      }
      return { content: [{ type: 'text' as const, text: fn(store, meta, staleProbeDeferred, args) }] }
    } catch (e: any) {
  ...
  server.registerTool('top_topics', {
  ...
  server.registerTool('describe_schema', {
    ...
    try {
      return { content: [{ type: 'text' as const, text: describeSchema(session.currentDir(), args) }] }
    } catch (e: any) {
      return { content: [{ type: 'text' as const, text: `error: ${e?.message ?? String(e)}` }], isError: true }
    }
  })
