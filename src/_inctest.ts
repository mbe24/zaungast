...
function craftDeletionLog(userKey: Buffer, seq: bigint): Buffer {
  const batch = Buffer.concat([
    (() => { const s = Buffer.alloc(8); s.writeBigUInt64LE(seq); return s })(),
    (() => { const c = Buffer.alloc(4); c.writeUInt32LE(1); return c })(), // count=1
    Buffer.from([0]),          // op type 0 = deletion
    varint(userKey.length), userKey,
  ])
  const type = Buffer.from([1]) // FULL record
  const crc = Buffer.alloc(4); crc.writeUInt32LE(maskCrc(crc32c(Buffer.concat([type, batch]), 0, 1 + batch.length)))
  const len = Buffer.alloc(2); len.writeUInt16LE(batch.length)
...
function dump(store: ChatStore): string {
  const db = store.db
  const msgs = db.prepare(`select conv_id,id,chain_key,version,ts,sender_mri,sender_name,kind,is_mine,is_system,has_attach,mentions_me,content
    from messages order by conv_id,id`).all()
  const convs = db.prepare(`select id,kind,topic,team_id,meta_last_ts,msg_count,participant_names,participant_count,activity_ts,last_ts
    from conversations order by id`).all()
...
function ftsConsistent(store: ChatStore): boolean {
  const db = store.db
  const a = db.prepare(`select group_concat(id) g from (select id from messages_fts order by id)`).get() as any
...
function msgStoreKey(live: any): Buffer | null {
  ...
  for (const e of live) { let p: any; try { p = decodePrefix(e.key) } catch { continue } if (targets.has(`${p.databaseId}:${p.objectStoreId}`)) return e.key }
  return null
}
...
function midSeq(dir: string): bigint {
  ...
  const targets: Set<string> = entityTargets(live, getMapping(live), 'message')
  const seqs: bigint[] = []
  for (const e of live) { let p: any; try { p = decodePrefix(e.key) } catch { continue } if (targets.has(`${p.databaseId}:${p.objectStoreId}`)) seqs.push(e.seq) }
...
console.log('\n=== A. spine: partial(seqCap) + incremental == full rebuild ===')
{
  ...
  console.log(`  partial had ${nBefore} msgs (cap seq ${cap}), incremental → ${nAfter}, full → ${(full.store.db.prepare('select count(*) n from messages').get() as any).n}`)
  ...
  partial.store.close(); full.store.close()
}
...
console.log('\n=== C. idempotency: incremental twice ===')
{
  ...
  applyIncremental(p.store, p.state!, DIR); p.state!.maxSeq = loadEntries(DIR).maxSeq
...
console.log('\n=== D. deletion: crafted tombstone for a chain → incremental deletes it == full of modified ===')
{
  ...
  ok('incremental (after deletion) == full rebuild of modified', dump(partial.store) === dump(fullMod.store))
  ok('FTS consistent after deletion', ftsConsistent(partial.store))
  partial.store.close(); fullMod.store.close()
...
console.log('\n=== E. Session end-to-end: warm full → mutate → incremental refresh ===')
{
  ...
  fs.writeFileSync(path.join(copy, 'zz0.log'), craftDeletionLog(msgStoreKey(live)!, maxSeq + 1000n))
  ...
  s.dispose(); fs.rmSync(copy, { recursive: true, force: true })
}
...
console.log('\n=== F. backstop: full every maxIncrementals refreshes ===')
{
  ...
  ok('backstop forces a full at the Nth incremental', modes[0] === 'incremental' && modes[1] === 'incremental' && modes[2] === 'full')
  s.dispose(); fs.rmSync(copy, { recursive: true, force: true })
}
...
console.log('\n=== G. multi-step: two effectful incrementals (add then delete) == full of final ===')
{
  ...
  applyIncremental(s.store, s.state!, copy); s.state!.maxSeq = loadEntries(copy).maxSeq   // step 1: catch up
  const n1 = (s.store.db.prepare('select count(*) n from messages').get() as any).n
  const { live, maxSeq } = loadEntries(copy)
  fs.writeFileSync(path.join(copy, 'zzz1.log'), craftDeletionLog(msgStoreKey(live)!, maxSeq + 1000n)) // step 2: delete
  applyIncremental(s.store, s.state!, copy); s.state!.maxSeq = loadEntries(copy).maxSeq
  ...
  s.store.close(); full.store.close(); fs.rmSync(copy, { recursive: true, force: true })
}
...
console.log('\n=== H. lossy load MUST NOT mass-delete (Hole 1 regression) ===')
{
  ...
  const buf = fs.readFileSync(p); fs.writeFileSync(p, buf.subarray(0, Math.floor(buf.length / 2))) // truncate → bad footer/index
  ...
  s.store.close(); fs.rmSync(corrupt, { recursive: true, force: true })
}

console.log('\n=== I. store-level: edit + soft-delete update flags and FTS (Hole 3) ===')
{
  const st = new ChatStore()
  const base = { convId: 'c', chainKey: 'k', ts: 1, senderMri: 'm', senderName: 'A', kind: 'chat', isMine: 0, hasAttach: 0, mentionsMe: 0 }
  st.insertMessage({ ...base, id: '1', version: 1, isSystem: 0, content: 'hello world' })
  st.refreshFts(null)
  ok('v1 findable in FTS', (st.db.prepare("select count(*) n from messages_fts where messages_fts match 'hello'").get() as any).n === 1)
  // edit: new content, same id, higher version
  st.insertMessage({ ...base, id: '1', version: 2, isSystem: 0, content: 'edited text' })
  st.refreshFts(null)
  ok('edit: old token gone', (st.db.prepare("select count(*) n from messages_fts where messages_fts match 'hello'").get() as any).n === 0)
  ok('edit: new token found', (st.db.prepare("select count(*) n from messages_fts where messages_fts match 'edited'").get() as any).n === 1)
  ...
  st.refreshFts(null)
  ok('soft-delete: is_system updated on conflict', (st.db.prepare('select is_system from messages where id=?').get('1') as any).is_system === 1)
  ok('soft-delete: dropped from FTS', (st.db.prepare("select count(*) n from messages_fts where messages_fts match 'edited'").get() as any).n === 0)
  st.close()
}
...
console.log('\n=== K. Session cold-start lossy full is flagged and self-heals ===')
{
  ...
  const s = new (Session as any)({ overrideDir: corrupt, minDebounceMs: 0, incrementalMode: 'reparse' })
  ...
  s.dispose(); fs.rmSync(corrupt, { recursive: true, force: true })
}
...
console.log('\n=== L. hardening: post-COMMIT recompute throw → needFullRebuild (not a stuck store) ===')
{
  ...
  fs.writeFileSync(path.join(copy, 'zzz2.log'), craftDeletionLog(msgStoreKey(loadEntries(copy).live)!, maxSeq + 1000n))
  const orig = s.store.recomputeDerived.bind(s.store)
  let thrown = false
  ;(s.store as any).recomputeDerived = () => { throw new Error('boom (simulated post-commit failure)') }
  ...
  let writable = true
  try { s.store.db.exec('BEGIN'); s.store.db.exec('COMMIT') } catch { writable = false }
  ok('store not left in an open transaction', writable)
  s.store.close(); fs.rmSync(copy, { recursive: true, force: true })
}
...
console.log('\n=== M. hardening: Session leaves no temp snapshot dir behind on a refresh throw ===')
{
  ...
  let threw = false
  try { s.refreshNow(true) } catch { threw = true }
...
console.log('\n=== N. hardening: handles stable across a full rebuild of identical data ===')
{
  const a = ingest(DIR)
  const b = ingest(DIR)
  const ha = (a.store.db.prepare('select id,handle from conversations order by id').all() as any[]).map((r) => `${r.id}=${r.handle}`).join('|')
  const hb = (b.store.db.prepare('select id,handle from conversations order by id').all() as any[]).map((r) => `${r.id}=${r.handle}`).join('|')
  ok('conversation handles reproducible across rebuilds', ha === hb)
  const pa = (a.store.db.prepare('select mri,handle from people order by mri').all() as any[]).map((r) => `${r.mri}=${r.handle}`).join('|')
  const pb = (b.store.db.prepare('select mri,handle from people order by mri').all() as any[]).map((r) => `${r.mri}=${r.handle}`).join('|')
  ok('people handles reproducible across rebuilds', pa === pb)
  ok('handles are 6-hex (c:xxxxxx)', /^c:[0-9a-f]{6}$/.test((a.store.db.prepare('select handle from conversations limit 1').get() as any).handle))
