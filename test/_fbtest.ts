...
const d = false
const busiest = store.db.prepare('select handle from conversations order by msg_count desc limit 1').get() as any
const botP = store.db.prepare("select handle from people where mri like '28:%' order by msg_count desc limit 1").get() as any
...
{
  const gen = store.db.prepare("select count(*) n from conversations where topic='General'").get() as any
  const res = search(store, meta, d, { in: 'General', limit: 2 })
  // only assert the note when the fixture actually has duplicate "General"s
  ok('duplicate in: titles noted', gen.n < 2 || /matched \d+ conversations/.test(res), `Generals=${gen.n}`)
}
...
console.log('=== exclude (words + handles) ===')
{
  ...
  ok('bad exclude handle fails loudly', /exclude: no (conversation|person)/.test(search(store, meta, d, { query: 'x', exclude: ['c:nope'] })))
}
...
console.log('=== top_topics since/until + P4 ===')
{
  ...
  ok('bad since errors', /cannot parse since/.test(topTopics(store, meta, d, { since: 'last week' })))
  ok('P4: nonexistent conversation scope errors', /no conversation matches/.test(topTopics(store, meta, d, { scope: 'conversation:zzzznope' })))
}
...
console.log('=== describe_schema nested labeling ===')
{
  ...
  ok('nested fields include the mapped paths', /content/.test(ds) && /imDisplayName/.test(ds) && /originalArrivalTime/.test(ds))
  ok('labels the record level', /record fields:/.test(ds))
}

console.log('=== read_messages header ===')
{
  ok('header shows local cache bounds', /local cache .+–.+/.test(readMessages(store, meta, d, { conversation: busiest.handle, limit: 2 })))
}
