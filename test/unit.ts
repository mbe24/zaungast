...
// @ts-ignore
import * as Snappy from '../src/format/snappy.js'
// @ts-ignore
import { deserialize } from '../src/format/ssv.js'
// @ts-ignore
import { crc32c } from '../src/format/sstable.js'
...
const eq = (n: string, a: unknown, b: unknown) => ok(n, JSON.stringify(a) === JSON.stringify(b), `got ${JSON.stringify(a)} want ${JSON.stringify(b)}`)
...
console.log('=== structured-clone (V8) ===')
{
  ...
  // {t:true, s:"hi"}
  const b2 = Buffer.from([0xff, 0x0f, 0x6f, 0x22, 0x01, 0x74, 0x54, 0x22, 0x01, 0x73, 0x22, 0x02, 0x68, 0x69, 0x7b, 0x02])
...
console.log('=== htmlToText ===')
{
  ...
  ok('control message is system', isSystemMessage({ type: 'Message', messageType: 'ThreadActivity/AddMember' }))
  ok('normal message is not system', !isSystemMessage({ type: 'Message', messageType: 'RichText/Html' }))
