...
export function keyPrefix(databaseId: number, objectStoreId: number, indexId: number): Buffer {
  if (databaseId > 255 || objectStoreId > 255 || indexId > 255) throw new Error('fixture ids must be < 256 (single-byte prefix only)')
...
const TAG = {
  NULL: 0x30, TRUE: 0x54, FALSE: 0x46, INT32: 0x49, DOUBLE: 0x4e,
  STR1: 0x22, BEGIN_OBJ: 0x6f, END_OBJ: 0x7b, BEGIN_DENSE: 0x41, END_DENSE: 0x24,
}

function encodeString(s: string): Buffer {
  for (let i = 0; i < s.length; i++) if (s.charCodeAt(i) > 0xff) throw new Error(`fixture strings must be latin1/ASCII: ${JSON.stringify(s)}`)
  return Buffer.concat([Buffer.from([TAG.STR1]), varint(s.length), Buffer.from(s, 'latin1')])
}

function encodeDouble(d: number): Buffer {
  const b = Buffer.alloc(8); b.writeDoubleLE(d); return Buffer.concat([Buffer.from([TAG.DOUBLE]), b])
}
...
export function encodeValue(v: unknown): Buffer {
  ...
  if (typeof v === 'number') {
    if (Number.isInteger(v) && Math.abs(v) < 2 ** 31) return Buffer.concat([Buffer.from([TAG.INT32]), zigzagVarint(v)])
...
function encodeBatch(sequence: number, ops: WalOpIn[]): Buffer {
  const seqBuf = Buffer.alloc(8); seqBuf.writeBigUInt64LE(BigInt(sequence))
  const countBuf = Buffer.alloc(4); countBuf.writeUInt32LE(ops.length)
  const opBufs: Buffer[] = []
  for (const op of ops) {
    if (op.type === 1) {
...
class LogWriter {
  ...
  addRecord(payload: Buffer): void {
    ...
    do {
      ...
      if (leftover < HEADER) {
        if (leftover > 0) { this.chunks.push(Buffer.alloc(leftover)); this.blockOffset += leftover }
