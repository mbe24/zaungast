...
export interface TableEntryIn { key: Buffer; value: Buffer } // key = userKey ++ 8-byte trailer (already appended)
interface BlockHandleOut { offset: number; size: number }
...
export function encodeTable(entries: TableEntryIn[]): Buffer {
  ...
  const indexContents = buildBlockContents([{ key: indexKey, value: blockHandleBytes(dataHandle) }])
  ...
  let pos = 0
  const mh = blockHandleBytes(metaHandle); mh.copy(footer, pos); pos += mh.length
  const ih = blockHandleBytes(indexHandle); ih.copy(footer, pos)
